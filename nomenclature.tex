...
\nomenclature{PIRL}{Programmatically Interpretable Reinforcement Learning \cite{vermaProgrammaticallyInterpretableReinforcement2018}, same as RLCEF}
\nomenclature{PSB2}{Program Synthesis Benchmark 2 \cite{helmuth2015:general}}
\nomenclature{RLCEF}{Reinforcement Learning from Code Execution Feedback, same as PIRL}
\nomenclature{RLCEPS}{Reinforcement Learning from Code Execution in Patient Simulators, introduced in chapter \ref{ch:proposal}}
...
\nomenclature{MPDP}{Message Passing Decision Process, introduced in section \ref{sec:mpdp}}
\nomenclature{KPI}{Key Performance Indicator}

\nomenclature{PG}{Policy Gradient\footnote{Unless referring to \cite{grahamHundredYearLanguage}}}
\nomenclature{PQT}{Priority Queue Training \cite{abolafiaNeuralProgramSynthesis2018}}
\nomenclature{SARSA}{State Action Reward State Action \cite[Chapter 6]{suttonReinforcementLearningSecond2018}}
\nomenclature{CEM}{Cross-Entropy Method \cite{szitaLearningTetrisUsing2006}}
\nomenclature{DQN}{Deep Q-Network \cite{mnihPlayingAtariDeep2013,mnihHumanlevelControlDeep2015}}
\nomenclature{PPO}{Proximal Policy Optimization \cite{schulmanProximalPolicyOptimization2017}}

\nomenclature{GPT}{Generative Pre-trained Transformer \cite{radfordImprovingLanguageUnderstanding}}
\nomenclature{BERT}{Bidirectional Encoder Representations from Transformers}
\nomenclature{LLM}{Large Language Model \cite{zhouComprehensiveSurveyPretrained2023}}
\nomenclature{RNN}{Recurrent Neural Network}
\nomenclature{LSTM}{Long Short Term Memory \cite{hochreiterLongShorttermMemory1997}}
\nomenclature{VAE}{Variational Autoencoder \cite{kingmaIntroductionVariationalAutoencoders2019}}
...
\nomenclature{ICU}{Intensive Care Unit \cite{WhatIntensiveCare2017}}
\nomenclature{HCPCS}{Healthcare Common Procedure Coding System \cite{ciminoCodingSystemsHealth2018}}
\nomenclature{eMAR}{Electronic Medication Administration Records}
\nomenclature{QALY}{Quality Adjusted Life Year \cite{ryenWillingnessPayQuality2015, torranceUtilitiesQualityadjustedLife1989}}
\nomenclature{MIMIC}{Medical Information Mart for Intensive Care \cite{johnsonMIMICIVVersion042020}}
...
\nomenclature{$\indicator(\text{condition})$}{indicator function, defined as 1 if \emph{condition} is true and 0 otherwise}
