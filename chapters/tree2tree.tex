...
Can we combine the simplicity of neurogenetic optimization in \emph{BF++} space with the advantages of a popular language \cite{tiobeTiobeIndex2017}?
One approach would be to train a foundational (section \ref{sec:pretrain}) autoencoder \cite{bankAutoencoders2020} model for code.
...
According to \cite{kusnerGrammarVariationalAutoencoder2017,kostajrProgramSynthesisVulnerability2019}, it is beneficial to include grammatical metadata in token representations for a traditional sequence model, but do not employ a tree-based encoder-decoder architechure.
...
Variational autoencoder \cite{kingmaAutoencodingVariationalBayes2013} is chosen since its Kullback-Leibner component encourages the model to use the smallest subspace of the latent space possible, making the model less brittle with respect to the dimensionality of the latent space than its deterministic counterpart.
...
We use a 3-layer architecture, in which the hidden state of a Tree-LSTM unit in layer $l$ is then used as input to the Tree-LSTM unit in layer $l + 1$ in the same time step, the same as with the standard LSTM \cite{gravesHybridSpeechRecognition2013}. 
...
The node importance calculation is based on \cite{winataAttentionbasedLstmPsychological2018}, and updates the hidden states as follows:
\begin{align}
...
We reject a common \cite{fabiusVariationalRecurrentAutoencoders2015} approach of only taking the last hidden state of the RNN as the input to the decoder due to the long-term memory loss problem \cite{kaoComparisonPoolingMethods2020} and use max pooling instead.
See fig. \ref{fig:pooling}.

\paragraph{Sampling latent code} The pooled vector is then used to compute the mean and variance of the approximate posterior to sample a latent code $\latent \sim \normaldistr(\mean, \stddev)$ with the help of the reparametrization trick \cite{kingmaAutoencodingVariationalBayes2013}. The mean and variance are computed using linear layers with learnable weights $\weights_{\mean}$, $\weights_{\stddev}$ and biases $\biases_{\mean}$, $\biases_{\stddev}$:
...
Having the order of the input sequence reversed compared to the reconstructed sequence has been shown in \cite{fabiusVariationalRecurrentAutoencoders2015} to improve the performance of the model. 
...
Applying DRNN \cite{alvarez-melisTreestructuredDecodingDoublyrecurrent2017} to code entails special advantages: grammatical constraints of the programming language can be used to narrow down the token vocabulary for the model to select from.
...
For literal tokens, the vocabulary can, in theory, be infinitely large. We employ adaptive softmax \cite{graveEfficientSoftmaxApproximation2017} to use a vocabulary consisting of many unique literal tokens without a considerable increase in computational complexity.
...
Since program behavior is invariant to identifier replacement, instead of attempting to accurately predict identifier names we map each unique identifier to a reusable ID \cite{tufanoLearningMeaningfulCode2019} and treat the prediction of identifiers as a clustering problem. 
...
We mitigate it using cyclical KL cost annealing \cite{fuCyclicalAnnealingSchedule2019}. 
Furthermore, we apply pooling to the hidden states of the RNN network in the encoder. Long \textit{et al.} \cite{longPreventingPosteriorCollapse2019} show this pooling method can effectively prevent posterior collapse. 
...
Lastly, since identifier prediction is a clustering problem, we can use triplet loss \cite{chechikLargeScaleOnline2010}. 
...
Our baseline model is a maximally simple sequence to sequence (Seq2Seq) autoencoder based on a single layer Gated Recurrent Unit \cite{chungEmpiricalEvaluationGated2014} architecture. Similar to Tree2Tree, we employ methods to mitigate KL-vanishing, namely, cyclic KL annealing \cite{fuCyclicalAnnealingSchedule2019} and word dropout \cite{bowmanGeneratingSentencesContinuous2015}.
...
However, non-leaf nodes of the AST have interpretable labels such as "function declaration", so we expect GLoVe \cite{penningtonGloveGlobalVectors2014} embeddings in the Tree2Tree model to be meaningful.
...
We choose BLEU \cite{papineniBleuMethodAutomatic2002} as the similarity metric to maintain consistency with recent code generation literature \cite[table I]{evtikhievOutBLEUHow2023}.
...
We employ two decoding strategies to test the generative capabilities of the models: greedy decoding and sampling. The sampling strategy we apply is a combination of top-$k$, nucleus, and temperature sampling \cite{holtzmanCuriousCaseNeural2019}. 
...
So, while Tree2Tree does advance the state of the art in autoencoders for code, for the purposes of RLCEPS (see chapter \ref{ch:proposal}) we have to concur with \cite{mcdermottWhyAutoencodingDifficult2019}: autoencoder genetic programming remains hard.
