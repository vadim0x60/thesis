...
We design the templates with prompt engineering guidelines~\cite{openaiPromptEngineering} and prior work from chapter \ref{ch:tree2tree} in mind.
...
Here, the repair-only scenario can also be seen as an LLM-guided random walk~\cite{xiaRandomWalksReview2019} and replace-only as random sampling from the LLM.
...
Until recently, the tasks of neural program synthesis~\cite{gulwaniProgramSynthesis2017} and program repair~\cite{gouesAutomatedProgramRepair2019,petke2018:genetic,legouesSystematicStudyAutomated2012,zhangSystematicLiteratureReview2024} have been considered separately.
...
This approach is meant for the cases where creative freedom~\cite{stanleyWhyGreatnessCannot2015} is encouraged rather than satisfying concrete requirements.
\cite{liuAlgorithmEvolutionUsing2023} demonstrate the advantages of applying ELM to modular components of a solution, rather than the entire solution to the given problem.

Some related work is explicitly guided by the metaphor of conversation between agents. 
A repair-only program synthesis loop can be implemented as a conversation between an analyst agent, a coder, and a tester~\cite{zhangSTEAMSimulatingInteractive2023} or between a tester, a developer, and a reviewer~\cite{dongSelfcollaborationCodeGeneration2023}. 
...
We define \emph{pass@k} as the number of problems that have $TPR=1$ if SEIDR is stopped after generating $k$ programs, following~\cite{kulalSpocSearchbasedPseudocode2019} \emph{``success rate at budget of $k$ programs.''}
...
Following~\cite{helmuthPSB2SecondProgram2021}, we use 2000 I/O pairs ($\left(I, O\right)_{test}$ in Figure~\ref{fig:method}) from the test split of PSB2 to evaluate the candidate program that has passed all validation test cases ($\left(I, O\right)_{val}$ in Figure~\ref{fig:method}) during debugging. 
...
\input{tables/generalizability-psb2-py}
\end{table}
...
\input{tables/generalizability-psb2-cpp}
\end{table}
...
\input{tables/generalizability-he-py}
\end{table}
...
\input{tables/generalizability-he-cpp}
\end{table}
