...
\begin{figure}
  \centering
  \includegraphics[width=0.8\textwidth, trim={3mm 1.6mm 3mm 2mm}, clip]{images/num_programs_generated_vs_bf_test_pass_rate_vertical_maxprog_1000_v3.pdf}
...
We will refer to the equally set values of $\treearity_\text{draft}$ and $\treearity_\text{debug}$ as $\treearity$ hereafter.
...
To explore the capabilities of SEIDR with the studied models, we can take a union over experiments and count the number of problems solved at least once across 36\footnote{~We have 6 runs and 6 different values of $\treearity.$} restarts with a fixed dataset, language and model.
...
\begin{table}[t]
    \centering
    \caption{Average number of PSB2 problems solved in \textbf{Python} using SEIDR with different LLMs and PushGP. The best results among SEIDR-only experiments at each $k$ are highlighted in bold. If pass@100 is not available, we cite the best available results in brackets.}\small
...
\begin{table}[t]
  \centering
  \caption{Average number of PSB2 problems solved in \textbf{C++} using SEIDR with different LLMs and PushGP. The best results among SEIDR-only experiments at each $k$ are highlighted in bold. If pass@100 is not available, we cite the best available results in brackets.}\small
...
\begin{table}[t]
    \centering
    \caption{Percentage HumanEval-X tasks solved in \textbf{Python} using SEIDR with different LLMs. The best results among SEIDR-only experiments at each $k$ are highlighted in bold.}\small
...
\begin{table}[t]
  \centering
  \caption{Percentage HumanEval-X tasks solved in \textbf{C++} using SEIDR with different LLMs. The best results among SEIDR-only experiments at each $k$ are highlighted in bold.}\small
...
We combine all generalizability results in tables \ref{tab:generalizability-psb2-py}, \ref{tab:generalizability-psb2-cpp}, {tab:generalizability-he-py} and \ref{tab:generalizability-he-cpp}, where we show the number of solved problems at different cutoffs ($k$).  
