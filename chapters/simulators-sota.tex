...
Patient simulators have been applied extensively in nursing education \cite{mcfetrichStructuredLiteratureReview2006,nehringHumanPatientSimulators2001,nehringCurrentUseOpinions2004}, both as physical mannequins and as digital patient simulation software \cite{leeEffectiveVirtualPatient2020}.
...
The term \emph{patient simulator} does not have one rigorous definition, but in present thesis we focus specifically on interactive systems where a (human or software) agent communicates its clinical intervention decisions to the simulator and the simulator, in turn,
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item synthetic data generators \cite{chenSyntheticDataMachine2021}
\end{enumerate}

The former are tools like HumMod \cite{hesterHumModModelingEnvironment2011} that help predict how change in one variable of patient health can affect another, for instance, how digoxin intake affects blood pressure.
...
They provide a privacy-preserving way to generate a lot of training data for treatment discovery \cite{gongSurveyDifferentiallyPrivate2020,wangGeneratingEvaluatingCrosssectional2021,jordonPATEGANGeneratingSynthetic2018,rankinReliabilitySupervisedMachine2020}, however, they do not model counterfactuals. That is, they do not answer questions of the sort "How would the patient's health respond to decision X"? For example, Synthea \cite{walonoskiSyntheaApproachMethod2017} is a framework for generation of entire synthetic Electronic Health Records, EHRs, of patients, from the first to the last patient encounter, but the system does not enable testing alternative interventions on patients.
...
One metric in particular that has been used for simulator validation is \emph{mean predictive error over dynamic transitions} \cite{summersValidationComputationalPlatform2009}.
...
Machine learning in Healthcare, an emergent field with unique challenges in availability of research datasets \cite{anshikHandlingAvailabilityLowTraining2021,gilbertMarketHealthcareData2015,pahwaBigDataMachine2021,yazhiniStateArtApproaches2019} lacks an accepted benchmarking standard: recent literature reviews \cite{palMachineLearningHealthcare2023,tortorellaHealthcare40Trends2020} of the field cover a variety of studies that each use their own (often non-public) benchmark. The shortage of standard benchmarks has been consistently identified as a central roadblock for machine learning in Healthcare
\cite{crownPotentialApplicationMachine2015,davidbellamyEvaluatingProgressMachine2020,guSupervisedLearningPervasive2023,harutyunyanMultitaskLearningBenchmarking2019,kathrinblagecBenchmarkDatasetsDriving2022,mcdermottReproducibilityMachineLearning2021,purushothamBenchmarkingDeepLearning2018,s.purushothamBenchmarkDeepLearning2017}.
...
Didactic simulators can be particularly good benchmarks (to be discussed in chapter \ref{ch:auto-als}), but synthetic games akin to CartPole \cite{bartoNeuronlikeAdaptiveElements1983} and MountainCar \cite{mooreEfficientMemorybasedLearning1990} (commonly used as benchmarks for Reinforcement Learning algorithms) that aim to mimic only certain aspects of real life healthcare tasks can also be used.
One particularly illustrative example is \emph{Healing MNIST} \cite{krishnanDeepKalmanFilters2015} - a modified version of the industry standard MNIST Handwritten Digits \cite{dengMNISTDatabaseHandwritten2012} dataset to which rotations and random noise have been added.
...
\emph{Simglucose}~\cite{xieSimglucoseV0212018} solves exactly that by adding a reward function based on diabetes risk index as defined in \cite{clarkeStatisticalToolsAnalyze2009} to the UVA/Padova simulator, providing a Reinforcement Learning environment for type 1 diabetes.
...
\subsection{Scope}
GYMIC \cite{kianiGYMICOpenAIGym} is, unlike examples in section \ref{sec:simglucose}, a fully data-driven simulator.
...
The authors intentionally limit their scope to just sepsis in order to simplify the modelling task as well as because sepsis prevention has been identified as an area where doctors would particularly benefit from electronic decision support \cite{raghuDeepReinforcementLearning2017,walonoskiDeepReinforcementLearning2018}.
...
Autoregressive tasks of this nature arise in many fields like stock market prediction \cite{cavalcanteComputationalIntelligenceFinancial2016,jiangApplicationsDeepLearning2021} or language modelling \cite{jozefowiczExploringLimitsLanguage2016} where state of the art solutions can be found.
...
Faced with some of the mode collapse issues described in section \ref{sec:gymic-results} the authors also experimented with semi-supervised learning \cite{learningSemisupervisedLearning2006}: they trained a variational autoencoder \cite{kingmaIntroductionVariationalAutoencoders2019} on all patient states to replace the 46-vector representations of patient state $\state$ with learned representations from the latent space of the VAE $\text{encoder}(\state)$.
...
However, in order to lower the difficulty of the simulator (delayed gratification makes training significantly harder \cite{mischelAttentionDelayGratification1970,gulwaniProgramSynthesis2017}) an additional reward is provided during the episode, based on the evolution of the patient's SOFA score \cite{lambdenSOFAScoreDevelopment2019} - a commonly used measure of sepsis severity:
