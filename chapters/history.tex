...
It provides $\prob_\obs(\obs | \state, \action)$ and $\prob_s(\state_\text{next} | \state_\text{prev}, \action)$ (see section \ref{sec:ps-task-taxonomy}), so to be a full-fledged Markov Decision Process it only need $\prob_r(r|\state,\action)$.
