...
Using methods from single variable calculus, one can derive the gradient of this function can be derived analytically:
...
Let us begin with an intitial population of 2 instances of $ x $ and, At every iteration of the evolutionary algorithm, draw $ x_1 $ and $ x_2 $ from the exponential reward distribution:
...
Repeat this offspring sampling procedure for 10000 iterations and from the resulting population of 10002 pick $ x $ with the maximal $ O(x) $.
...
This could be mitigated by various means, for instance, \emph{temperature sampling} adding a temperature parameter to $ p(x) $:
