...
\begin{remark}
    An earlier revision of this chapter \cite{liventsevNeurogeneticProgrammingFramework2021,liventsevNeurogeneticProgrammingFramework2021} was presented at GECCO 2021
\end{remark}
...
This function is visualized in figure \ref{fig:neuropt-f}
...
Note that the darker colored points of the visualization in figure \ref{fig:neuropt-search} were added to the population after the light colored ones. 
...
This could be mitigated by various means, for instance, \emph{temperature sampling} \cite{holtzmanCuriousCaseNeural2019}, namely, adding a temperature parameter to $ \prob(\mlinput) $: $\policy_t(\mlinput) \sim e^{t\obj(\mlinput)}$.
...
In figure \ref{fig:neuropt-combined} we do exactly that: add one element to the population by sampling it from the actor distribution $ \mlinput_{\learnables}(\latent) $, update the actor and the critic, then sample a pair $\langle x1,x2 \rangle$ as we did in the evolutionary approach and add their average.
One can see that in the hybrid neural-genetic mode, the search process converges substantially faster and finds it within 4000 iterations, faster than both pure neural and pure genetic approaches 

\newpage
\section{Neurogenetic Optimization for Code}

In \emph{genetic programming} \cite{banzhafGeneticProgrammingIntroduction1998,poliFieldGuideGenetic2008} new programs are generated by mutating and mixing a \emph{population} of programs. 
 A more recent approach, largely drawing on the earlier success of deep neural language models (see CodeBERT \cite{fengCodebertPretrainedModel2020} inspired by BERT \cite{devlinBERTPretrainingDeep2019}), have been to train black box \emph{neural models} that generate executable programs as text \cite{abolafiaNeuralProgramSynthesis2018,balogDeepCoderLearningWrite2016,alonStructuralLanguageModels2020}. 
 Neural program synthesis and genetic programming both have unique advantages \cite{weiseAlgorithmSynthesisDeep}. 
 In this chapter we propose a novel hybrid of the two families of methods. We call the method \emph{Instant Scrum} in reference to a popular Agile software team work model \cite{schwaberAgileSoftwareDevelopment2002}. We show that \emph{Instant Scrum}, IS, can solve several reinforcement-guided program synthesis tasks in standard OpenAI gym benchmarks tasks (section \ref{sec:tasks}). 
...
This question is studied extensively in software project management literature \cite{brooksjrMythicalManmonthEssays1995}.
And while, admittedly, project management literature is concerned with human developers and, admittedly, there exist considerable differences between human developers and mathematical models of code generation \cite{legouesSystematicStudyAutomated2012}, we mitigate these differences with several simplifying assumptions.
...
The idea behind exponentiating the total reward is to encourage \emph{exploration} \cite{ruckstiessExploringParameterSpace2010}.
...
Just like \emph{instant run-off voting} achieves similar results to \emph{exhaustive ballot runoff voting}, but does it much faster by replacing a series of ballots cast in a series of elections with a ballot cast once that goes on to participate in a series of virtual elections \cite{devlinVotingSystemsMethod}, our \emph{Instant Scrum} algorithm does the same to Scrum \cite{schwaberAgileSoftwareDevelopment2002}: it simulates the iterative software development process recommended by Scrum methodology without humans in the loop making it possible to run many sprints per second:
...
The choice of operators follows from DEAP \cite{fortinDEAPEvolutionaryAlgorithms2012,derainvilleDEAPEnablingNimbler2014,derainvilleDEAPPythonFramework2012} which we also use to implement the experiments.
...
In DNA \cite{morganCritiqueTheoryEvolution1916}, as well as in most genetic programming literature \cite{banzhafGeneticProgrammingIntroduction1998,poliFieldGuideGenetic2008} the crossover operator combines 2 parent sequences to produce 2 children.
...
To do this, we notice that equation \ref{eq:genmixture} contains a \emph{multi-armed bandit} \cite{robbinsAspectsSequentialDesign1952} hiding in plain sight.
...
This subproblem can be represented with a POMDP of its own and solved using one of the standard bandit algorithms \cite{kuleshovAlgorithmsMultiarmedBandit2014}.
...
where $\greed$ is a hyperparameter that controls the \emph{exploration-exploitation tradeoff} \cite{macreadyBanditProblemsExploration1998}
...
$\hidden_i(\learnables)$ can also represent several LSTM layers stacked or a different type of recurrent neural network, i.e. GRU \cite{choPropertiesNeuralMachine2014,chungEmpiricalEvaluationGated2014}.
...
Following from the previous chapter we synthesize programs for \textbf{CartPole-v1} \cite{bartoNeuronlikeAdaptiveElements1983}, MountainCarContinuous-v0 (\textbf{MCC-v0}) \cite{mooreEfficientMemorybasedLearning1990}, \textbf{Taxi-v3} \cite{dietterichHierarchicalReinforcementLearning2000} BipedalWalker-v2 (\textbf{BW-v2})  Gymnasium \cite{towersGymnasiumStandardInterface2024} environments, see figure \ref{fig:envs}.
...
For other tasks we used Exponential Variance Elimination \cite{liventsevVadim0x60EvestopEarly2021} early stopping algorithm to stop the process when the positive trend in $\returntot$ is not present for 10000 sprints.
...
% TODO [R2]: Add a labeled \section{Conclusion} summarizing what this chapter achieved and discussing limitations/future work.
