...
The right half of the search space remained unexplored - the rightmost solution $ \mlinput_\text{right} $ has an $ O(\mlinput_\text{right})=-2 $ which means it's almost never drawn from $ \prob(\mlinput) $. 
This could be mitigated by various means, for instance, \emph{temperature sampling} \cite{holtzman2019curious}, namely, adding a temperature parameter to $ \prob(\mlinput) $: $\prob_t(\mlinput) \sim e^{t\obj(\mlinput)}$.
...
Thus, a developer is a 2-tuple of a program distribution $\prob_\text{dev}(c | \learnables, \codebase)$ and a parameter update procedure $\mathit{Update}(\learnables, \code, \obj)$

Distribution $\prob_{\text{dev}}(c | \learnables, \codebase)$ is defined over programs and is parametrized with learnable parameters $\learnables$ as well as codebase $\codebase$. 
...
For this reason we train developers to maximize $\obj$, but when the training is complete, we pick programs from the codebase with the highest $R$ as "best programs".
...
\State $\code_\text{new}\sim \prob_i(\code | \learnables_i, \codebase)$ 
...
\begin{enumerate}
    ...
    \item Selecting two programs from the \emph{codebase} $C$ (parents $\code_1$ and $\code_2$) 
    \item Using the operator to modify the parents and yield a new (child) program
\end{enumerate}

A genetic operator is a probability distribution $\prob_\text{op}$ over child programs given 2 parent programs. 

\begin{equation}
    \prob_\text{op}(\code_\text{child}|\code_1,\code_2)
\end{equation}

Operators whose $\prob_\text{op}$ is invariant to $\code_2$ and depends on $\code_1$ only are called \emph{mutation} operators: they generate a new program by \emph{mutating} one program $\code_1$.
...
\begin{equation}
    \prob_\text{shuffle}(\code_\text{child}|\code_1,\code_2) =
...
With probability $\prob_\text{ind}$ it is replaced with a random token from the alphabet $\mathcal{L}$ of the programming language, with probability $1-\prob_\text{ind}$ it stays the same.
The evolution of a single token under shuffle mutation is defined by distribution

\begin{equation}
    \prob(\code^\text{new} | \code^\text{old}) = \frac{\prob_\text{ind}}{|L|} +  (1 - \prob_\text{ind}) \mathbb{I}[\code^\text{new} = \code^\text{old}]
\end{equation}

Hence over full programs the operator is defined as

\begin{equation}
    \prob_\text{unimut}(\code_\text{child}|\code_1,\code_2) = \mathbb{I}[|\code_\text{child}|=|\code_1|] \\ 
    \prod\limits_{\step=0}^{|\code_1|}  \left(\frac{\prob_\text{ind}}{|L|} +  (1 - \prob_\text{ind}) \mathbb{I}[\code_\text{child}^{(\step)} = \code_1^{(\step)}] \right)
\end{equation}
...
\begin{equation}
    \prob_\text{1ptcx}(\code_\text{child}|\code_1,\code_2) =
...
\begin{multline}
    \prob_\text{2ptcx}(\code_\text{child}|\code_1,\code_2) =
...
\emph{Uniform crossover} mirrors \emph{uniform mutation} in that a loaded coin is tossed for each token in $\code_1$. With probability $\prob_\text{ind}$ the token is replaced, but the replacement is not drawn randomly from the alphabet. Instead, the replacement comes from $\code_2$:

\begin{equation}
    \prob_\text{unicx}(\code_\text{child}|\code_1,\code_2) = \prod\limits_{\step=0}^{|\code_1|} \mathbb{I}[|\code_\text{child}|=|\code_1|] \\ \left(\prob_\text{ind} \mathbb{I}[\code_\text{child}^{(\step)} = \code_2^{(\step)}] + (1 - \prob_\text{ind}) \mathbb{I}[\code_\text{child}^{(\step)} = \code_1^{(\step)}] \right)
\end{equation}
...
\begin{equation}
    \prob_\text{prune}(\code_\text{child}|\code_1,\code_2)= \begin{cases}
...
Let $\mathcal{P}_\text{genetic}$ be a tuple of all available genetic operators, in order of introduction, i.e. $\mathcal{P}_\text{genetic}^{(1)}=\prob_\text{shuffle}$ and $\mathcal{P}_\text{genetic}^{(4)}=\prob_\text{2ptcx}$
...
\begin{equation}
    \prob_\text{genetic}(\code | \learnables, \codebase) = 
    \sum\limits_{\code_1}^{C}  
    \sum\limits_{\code_2}^{C} 
    \frac{\obj(\code_1|\codebase) \obj(\code_2|\codebase)}{(\sum\limits_{\code}^{C} \obj(\code|\codebase))^2} 
    \sum\limits_{\step=0}^{|\mathcal{P}_\text{genetic}|} 
    \learnables_i \mathcal{P}_\text{genetic}^{(\step)} (\code|\code_1,\code_2)
    \label{eq:genmixture}
\end{equation}

This is a true probability distribution if and only if $\sum\limits_{\step=0}^{|\mathcal{P}_\text{genetic}|} 
    \learnables_i = 1$
...
\begin{equation}
    V^{(\step)} = \frac{1}{|\codebase(\mathcal{P}_\text{genetic}^{(\step)})|} 
    \sum\limits_{k=1}^{|\codebase(\mathcal{P}_\text{genetic}^{(\step)})|}
    \codebase(\mathcal{P}_\text{genetic}^{(\step)})_R^{(k)} 
\end{equation}

where $C(\mathcal{P}_\text{genetic}^{(\step)})$ is the subset of the codebase produced via operator $\mathcal{P}_\text{genetic}^{(\step)}$.

The $\mathit{Update}_\text{genetic}$ procedure recalculates values $V$ and sets operator probabilities to

\begin{equation}
    \learnables_i = \frac{\epsilon}{|\mathcal{P}_\text{genetic}^{(\step)}|} +
    \mathbb{I}[\step = \underset{\step}{\arg\max} V^{(\step)}] (1 - \epsilon)
\end{equation}
...
\begin{enumerate}
    \item $\prob_\text{ind}$ defines severity of mutation in $\prob_\text{unimut}$ and $\prob_\text{unicx}$
...
The \emph{neural developer}, also known as the \emph{senior developer} because of their unique ability to write original programs, is an LSTM \cite{hochreiterLongShorttermMemory1997} network followed by a linear layer that generates a sequence of vectors $h_{1},h_{2},h_{3},\dots$ where $h_i \in \mathbb{R}^{|\mathcal{L}| + 1} \forall \step$ and $j$-th element of vector $h_i$, $h_i^{(j)}$, represents the probability of $\step$-th token of the program being $j$-th token in the alphabet, $\prob(\code^{(\step)}=\mathcal{L}^{(j)})$.
The last element of the vector represents a special \emph{end of program} symbol.
This vector depends deterministically on the full set of neural network parameters (LSTM and linear layer) $\learnables$ and can be represented as a function $h_i(\learnables)$.
Then

\begin{equation}
    \prob_\text{neural}(\code | \learnables, \codebase) = h_{(|\code|+1)}^{\mathcal{L}+1}
    \prod\limits_{\step=1}^{|\code|}
    \sum\limits_{j=1}^{|\mathcal{L}|} \mathbb{I}[\code^{(\step)}=\mathcal{L}^{(j)}]
    h_i(\learnables)
\end{equation}

For the $\mathit{Update}_\text{neural}$ procedure we use the algorithm proposed in \cite{abolafiaNeuralProgramSynthesis2018}.
The subproblem of generating a program $\code$ is considered as a reinforcement learning episode of it's own, where tokens are actions and token number $|\code|+1$ (\emph{end of program} token) is assigned reward $\obj = e^R; R \sim Eval(\code)$. 
In this subenvironment $h_i(\learnables)$ is the policy network \cite[chapter 13]{suttonReinforcementLearningSecond2018} trained using REINFORCE algorithm with Priority Queue Training.
This algorithm involves a priority queue of best known programs: we implement it as programs from $C$ with highest $\obj(\code|\codebase)$ which means that the neural developer can train on programs written by other developers.

$h_i(\learnables)$ can also represent several LSTM layers stacked or a different type of recurrent neural network, i.e. GRU \cite{choPropertiesNeuralMachine2014,chung2014empirical}.
...
\begin{equation}
    \prob_\text{dummy}(\code_\text{child}|\code_1,\code_2) = 
    \frac{\obj(\code_\text{child}|\codebase)}{\sum\limits_{\code}^{C} \obj(\code|\codebase)} 
    \label{eq:dummy}
\end{equation}
...
Genetic developers are denoted as $\text{gen}(\prob_\text{ind},\epsilon)$, see section \ref{sec:genhyper}.
...
For Taxi we set an $N_\text{max}$ to $100000 |T|$ sprints where $|T|$ is the team size, meaning every developer in the team trains for 100000 iterations.
...
Taxi-v3 & & -148.23 & $\text{gen}(0.2,0.2)$, $\prob_\text{unicx}$ \\
Taxi-v3 & \checkmark & -150.44 & human \\
