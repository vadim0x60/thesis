...
As such, it requires robust real time monitoring of the patient's vital signs due to quickly identify potential deterioration \cite{baileyTrialRealtimeAlert2013,blountRealTimeAnalysisIntensive2010,bockholtRealTimeMonitoringBlood2022,dimitriosg.katehakisDistributedAgentBasedArchitecture1999,friedStatisticalMethodsIntensive2000,maoIntegratedDataMining2012,prgometVitalSignsMonitoring2016,vincentImprovingDetectionPatient2018}.
...
% Datasets
MIMIC IV \cite{johnsonMIMICIVFreelyAccessible2023}, AmsterdamUMCdb \cite{amsterdamumcdb}, HiRID \cite{yecheHiRIDICUBenchmarkComprehensiveMachine} and the eICU Collaborative Research Database \cite{pollardEICUCollaborativeResearch2018} are databases of health records obtained from Intensive Care Units.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{mimiciv.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{mimicseq.png}
    \caption{Entity-Relationship diagram of MIMIC-SEQ}
    \label{fig:mimicseq-er}
\end{figure}
...
\begin{table}
    \centering
      \begin{tabular}{lcccc} \toprule{clustering} & {recall} & {accuracy} & {precision} & {F1}  \\ \midrule{c10}  & 0.903 & 0.840 & 0.790 & 0.827 \\
...
\begin{table}
    \centering
      \begin{tabular}{lcccc} \toprule{clustering} & {recall} & {accuracy} & {precision} & {F1}  \\ \midrule{c10}  & 0.942  & 0.854 & 0.850  &  0.890 \\ 
  ...
  \begin{table}
    \centering
      \begin{tabular}{lcccc} \toprule{configuration} & {recall} & {accuracy} & {precision} & {F1}  \\ \midrule{1}  & 0.505  & 0.996 &  0.727  & 0.595 \\
