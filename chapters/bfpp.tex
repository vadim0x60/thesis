...
State of the art Reinforcement Learning from Code Execution Feedback work \cite{abolafiaNeuralProgramSynthesis2018} identifies BF\footnote{Brainfuck} \cite{mullerBrainfuckEightinstructionTuringcomplete1993} as a promising program synthesis language for arbitrary reinforcement learning environments for the following reasons:
\begin{itemize}
...
BF's runtime model is inspired by the classic Turing Machine \cite{turingComputableNumbersApplication1938}: at any point during the program's execution, the state of the program consists of:
...
For such observation spaces, \textbf{BF++} uses a \textit{fluid discretization} system that learns the true distribution of observations online. The idea was inspired by a work of Touati {\sl et al} \cite{touatiZoomingEfficientModelfree2020}, although, they assumed that $\obss^\steppp$ has a finite diameter and did not support unbounded observation spaces.
...
With a low $\discretebins$ a lot of the information observed form the environment is lost, while when $\discretebins$ is in the hundreds the generated programs can become very complex.
...
An extension of our methodology to high-dimensional POMDPs (such as Atari games \cite{bellemareArcadeLearningEnvironment2013}, where the observation is a matrix of pixels on simulated game screen) can be achieved by adding a scene encoder neural network that maps the observed image to a low-dimensional vector as proposed in \cite{kimuraDAQNDeepAutoencoder2018}.
...
\begin{enumerate}
    ...
    \item Employ the author of the present thesis to write \textbf{BF++} programs to solve them
...
\begin{enumerate}
\item \textbf{CartPole-v1} \cite{bartoNeuronlikeAdaptiveElements1983}.
...
\item MountainCarContinuous-v0 (\textbf{MCC-v0})\cite{mooreEfficientMemorybasedLearning1990}.
...
\item \textbf{Taxi-v3} \cite{dietterichHierarchicalReinforcementLearning2000}. There are 4 locations (labeled by different letters) and the goal is to pick up the passenger at one location and drop him off in another in as few timesteps as possible spending as little fuel as possible.
...
\begin{figure}
    ...
    \caption{Neural-symbolic learning cycle \cite{baderDimensionsNeuralsymbolicIntegrationa2005}}
    \label{fig:cycle}
\end{figure}
...
Unlike in most neural-symbolic systems \cite{besoldNeuralsymbolicLearningReasoning2017} that extract knowledge from connectionist systems with algortihms like TREPAN \cite{francaRelationalKnowledgeExtraction2015} or JRip extraction \cite{svatosRevisitingNeuralsymbolicLearning2019}, the \emph{extraction} step is trivial since the neural network outputs a symbolic program directly.

In all experiments below, the \emph{writer agent}'s LSTM has hidden size of 50, batch size of 4 and is trained with RMSProp \cite{tielemanLecture65RmsPropDivide2012} optimizer.

\subsection{Stopping and Scoring}

All experiments were run with an upper limit of 100000 training episodes.
Environments other than \textbf{Taxi} also used Exponential Variance Elimination \cite{liventsevVadim0x60EvestopEarly2021} early stopping technique - training was stopped when the postive trend in the quality of the best found program stopped, i.e. when the exponential moving average of program quality is lower that it was 1000 episodes ago.
...
\begin{highlight}
The results in the OpenAI gym test examples show that the proposed system is able to find a functional solution to the problem. 
In some cases the performance is similar to the best deep learning solution but the obtained program remains still explainable.
 This is encouraging result suggests that the use of program induction methods may indeed be a viable way towards explainable solutions in RL applications. 
\end{highlight}
