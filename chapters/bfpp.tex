...
BF is indeed simple and general, but not so simple that a randomly generated BF program will be valid and meaningful. 
...
\begin{enumerate}
    ...
    \item Execute one write operation for every read operation
\end{enumerate}

Due to these limitations, as well as the lack of a general enough input output system in BF, the framework proposed in \cite{abolafiaNeuralProgramSynthesis2018} is only applicable in \emph{programming by example} settings such as string manipulation tasks, despite using Reinforcement Learning techniques.
In this chapter we propose an extension of BF with additional constructs that address these limitations and make BF++, the proposed language, suitable for programmatically interpretable reinforcement learning in an arbitrarily Partially Observable Markov Decision Process (POMDP) environment.
