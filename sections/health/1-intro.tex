...
In this paper we review the field of interactive patient simulators and evaluate their potential to be used for automated treatment discovery.
...
Any interactive simulator with a prediction model and a reward model (and thus, all simulators in this paper) can be represented as {\em Episodic Partially Observable Markov Decision Process} (EPOMDP) as described in \cite{neurogenetic}:
...
Markov Decision Processes are a standard formalism in the field of Reinforcement Learning \cite{thebook}, so standard RL algorithms can be used in conjunction with any simulator from this paper for treatment discovery.
...
A reward model is absolutely necessary for the application area that we focus on in this paper - development and validation of treatment strategies.
