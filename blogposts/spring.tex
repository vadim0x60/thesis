\section{Spring sampling for fast informed search}
...
For example, a model for ImageNet classification\footnote{\url{https://paperswithcode.com/sota/image-classification-on-imagenet) us a distribution $p(i,c}}$ indicating how likely it is that image $i$ belongs to class $c$.
Likewise, language models are probability distributions $p(w_1,\dots,w_n)$ indicating how likely it is that sentence $w_1,\dots,w_n$ is appears in the training corpus and/or satisfies the RLHF\footnote{\url{https://en.wikipedia.org/wiki/Reinforcement\emph{learning}from\emph{human}feedback}} annotator.
...
We use this technique](https://github.com/vadim0x60/seidr/blob/037c0f262921d88a22ae5b2888781082a891a0e2/seidr/llm.py#L112) in our program synthesis framework ([code](https://github.com/vadim0x60/seidr), [paper\footnote{\url{/publications/seidr)}} to sample programs that pass a given test suite.
